...
def train(env_name='CartPole-v1', episodes=1000, gamma=0.99, lr=1e-2, max_steps=500):
    ...
    policy_net = LinearPolicy(input_dim=4, hidden_dim=128, output_dim=2)
    ...
    try:
        ...
        env.close()
        with open("../examples/rewards_log_linear.json", "w") as f:
            json.dump(total_rewards, f)
        print("Training complete. Rewards saved to rewards_log_linear.json")
